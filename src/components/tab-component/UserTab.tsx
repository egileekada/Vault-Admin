...
import AboutUser from '../user-component/user-modal/AboutUser'
import BannedMessage from '../user-component/user-modal/BannedMessage'
import TransactionHistory from '../user-component/user-modal/TransactionHistory'
import UpdateMessage from '../user-component/user-modal/UpdateMessage'
        ...
        <div className='w-full p-8 overscroll-y-auto' >
            <div className='w-full flex items-center bg-white rounded-md py-6 px-4' >
                ...
                <div className='w-full ml-4 ' >
                    <p className='font-Inter-Bold text-lg' >USERS</p>
                    <p style={{color: '#979797'}} className='font-Inter-Regular text-sm ' >Manage and modify users</p>
                </div>
                ...
                <table className='text-sm '>
                    <thead>
                        <tr className=' font-Inter-Bold text-sm border-b border-gray-200' >
                            ...
                            <th className='bg-white'>Wallet Bal.</th>
                            <th className='bg-white'>Last Trans.</th>
                            <th className='bg-white'>More</th>  
                        </tr>
                    </thead>
                    <tbody>
                        {Information.map((item, index) => {
                            return( 
                                <tr key={index} className='font-Inter-Medium text-xs' > 
                                    ...
                                    <td>{item.date}</td>
                                    <td  >  
                                        <div onClick={()=> setShowModal(1)} className='w-full flex cursor-pointer ' > 
                                            <p style={{border: '1px solid #000', color: '#002343'}} className='font-Inter-Medium text-sm w-24 py-1 flex justify-center items-center rounded-md' >See more</p> 
                                        </div>
                                    </td>
                                </tr> 
                            )
                ...
                {showModal === 2 ? 
                    ...
                    {showModal === 3 ? 
                        (
                            <>
                                <div className="h-auto flex justify-center items-center overflow-x-hidden overflow-y-hidden fixed inset-0 z-50 outline-none focus:outline-none"> 
                                    <UpdateMessage close={setShowModal} />
                                </div> 
                                <div className="opacity-20 fixed flex flex-1 inset-0 z-40 bg-black"/>
                            </>
                        ) : null}
                        {showModal === 4 ? 
                            (
                                <>
                                    <div className="h-auto flex justify-center items-center overflow-x-hidden overflow-y-hidden fixed inset-0 z-50 outline-none focus:outline-none"> 
                                        <TransactionHistory close={setShowModal} />
                                    </div> 
                                    <div className="opacity-20 fixed flex flex-1 inset-0 z-40 bg-black"/>
                                </>
                            ) : null}
                            {showModal === 5 ? 
                                (
                                    <>
                                        <div className="h-auto flex justify-center items-center overflow-x-hidden overflow-y-hidden fixed inset-0 z-50 outline-none focus:outline-none"> 
                                            <BannedMessage close={setShowModal} />
                                        </div> 
                                        <div className="opacity-20 fixed flex flex-1 inset-0 z-40 bg-black"/>
                                    </>
                                ) : null}
